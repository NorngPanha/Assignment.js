        ...
        contacts.forEach(contact => {
            const newRow = `
                <tr id="contact-${contact.id}">
                    ...
                    <td>${contact.date}</td>
                    <td>
                        <button type="button" class="btn btn-primary btn-sm" onclick="editContact(${contact.id})">Edit</button>
                        <button type="button" class="btn btn-danger btn-sm" onclick="removeContact(${contact.id})">Delete</button>
                    </td>
                </tr>
            `;
    ...
    window.editContact = function (id) {
        ...
        if (index !== -1) {
            ...
            if (name && email) {
                ...
                const now = new Date();
                contacts[index].date = now.toLocaleDateString('en-US', { year: 'numeric', month: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric' });
                saveContactsToLocalStorage();
                renderContacts();
            } else {
                alert('Please fill out all fields.');
